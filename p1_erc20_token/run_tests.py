...
class TestERC20Challenge(unittest.TestCase):
    ...
    def test_payability(self):
        ...
        with self.assertRaises(web3.exceptions.Web3ValidationError):
            self.contract.functions.withdraw(0).transact({"from": self.account1, "value":2, "gas":200000})
        ...
        #with self.assertRaises(web3.exceptions.Web3ValidationError):
        self.contract.functions.totalSupply().transact({"from": self.account1, "value": 2, "gas": 200000})
    ...
    def test_raw_logs(self):
        ...
        transfer_event_filter = self.contract.events.Transfer.create_filter(fromBlock='latest')
        approval_event_filter = self.contract.events.Approval.create_filter(fromBlock='latest')
